...
/** Canonical quarter values. Lowercase end-to-end to match the solver
 *  (`config.VALID_QUARTERS`), professor `available_quarters`, and the
 *  default offerings JSON. The dropdown capitalizes for display only —
 *  state and the wire stay lowercase. Lives here (not in QuarterSchedule.tsx)
 *  so the file can export only its component — react-refresh/only-export-
 *  components otherwise errors. */
